...
function _longitude(t) {
  ...
  const sin_3g = (3 - 4 * sin_g * sin_g) * sin_g;
  return Q0 + Q1 * t + L0 * sin_g + L1 * sin_2g + L2 * sin_3g;
}
...
function _longitude_prime(t) {
  ...
  const cos_3g = 2 * cos_g * cos_2g - cos_g;
  return Q1 + G1 * (L0 * cos_g + 2 * L1 * cos_2g + 3 * L2 * cos_3g);
}
...
class Location {
  constructor(sin_hour_angle, cos_hour_angle, sin_elevation, azimuth) {
...
class Time {
  ...
  observer(lat, lon) {
    ...
    return new Location(
      ...
      _azimuth(sin_lat, cos_lat, tan_dec, sin_hour_angle, cos_hour_angle)
    );
  }
}
