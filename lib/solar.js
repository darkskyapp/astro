...
class Location {
  ...
  get elevation() {
    return Math.asin(this.sin_elevation) * DEG;
  }
}
...
class Time {
  ...
  observer(lat, lon) {
    ...
    const hour_angle = sidereal.local(julian.to(this.time), lon) * DEG - this.right_ascension;
    ...
    return new Location(
      hour_angle,
      sin_lat * sin_dec + cos_lat * cos_dec * Math.cos(hour_angle * RAD)
    );
  }
}
