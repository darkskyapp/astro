...
// Externally we work with millisecond timestamps and degrees.

function longitude(ms) {
  return _longitude(julian.to(ms)) * DEG;
}

function inverse_longitude(l) {
  return julian.from(_inverse_longitude(l * RAD));
}

function obliquity(ms) {
  return _obliquity(julian.to(ms)) * DEG;
}
...
function declination(ms) {
  ...
  return Math.sin(_longitude(t)) * Math.sin(_obliquity(t)) * DEG;
}

function right_ascension(ms) {
  const t = julian.to(ms);
  const l = _longitude(t);
  return Math.atan2(Math.sin(l) * Math.cos(_obliquity(t)), Math.cos(l)) * DEG;
}

// Solar position class. Helpful if you need to cache results!
class Solar {
  ...
  declination() {
    return Math.asin(this.sin_declination) * DEG;
  }
  ...
  hour_angle(lon) {
    return sidereal.local(julian.to(this.time), lon * RAD) * DEG - this.right_ascension;
  }
  ...
  sin_elevation(lat, lon) {
    ...
    const cos_dec = Math.sqrt(1 - sin_dec * sin_dec);
    return sin_lat * sin_dec + cos_lat * cos_dec * Math.cos(this.hour_angle(lon) * RAD);
  }

  elevation(lat, lon) {
    return Math.asin(this.sin_elevation(lat, lon)) * DEG;
  }
  ...
  static create(ms) {
    ...
    return new Solar(
      ...
      Math.atan2(sin_l * cos_e, cos_l) * DEG // right ascension
    );
  }
}

function noon(ms, lat, lon) {
