...
function _declination(t) {
  return Math.asin(_sin_declination(t));
}
...
// Suncalc.js https://github.com/mourner/suncalc/blob/a617534553b4ff69a267b75cbdb42bbecf2adbef/suncalc.js#L41
function azimuth(ms, lat, lon) {
  ...
  const lst = sidereal.local(t, lon * RAD);
  const sin_lst = Math.sin(lst);
  const cos_lst = Math.cos(lst);
  return Math.atan2(sin_lst, cos_lst * sin_lat - tan_dec * cos_lat);
}
