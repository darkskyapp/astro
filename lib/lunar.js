...
function rise_and_set(t, lat, lon) {
  ...
  const times = {moonrise: NaN, moonset: NaN};
  // Go in 2 hour chunks.
  for (let i=0; i <= 24; i+=2) {
    ...
    if (roots === 1) {
      if (h0 < 0 && isNaN(times.moonrise)) {
        times.moonrise = i + x1;
      } else if(isNaN(times.moonset)) {
        times.moonset = i + x1;
      }
    } else if (roots === 2) {
      times.moonrise = isNaN(times.moonrise) ? i + (ye < 0 ? x2 : x1) : times.moonrise;
      times.moonset  = isNaN(times.moonset) ? i + (ye < 0 ? x1 : x2): times.moonset;
    }

    // Found all the things!
    if(!isNaN(times.moonrise) && !isNaN(times.moonset)) {
      if(times.moonrise >= 24) {
        times.moonrise = NaN;
      } else {
        times.moonrise = hours_later(t, times.moonrise);
      }
      if(times.moonset >= 24) {
        times.moonrise = NaN;
      } else {
        times.moonset = hours_later(t, times.moonset);
      }
      return times;
    }
...
function rise(t, lat, lon) {
  return rise_and_set(t, lat, lon)[RISE];
}
/* In the next 24 hours */
function set(t, lat, lon) {
  return rise_and_set(t, lat, lon)[SET];
}
