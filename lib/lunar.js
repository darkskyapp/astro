...
function _rise_and_set(t, lat, lon) {
  ...
  for (let i=0; i <= 24; i+=2) {
    if (i !== 0) {
      h1 = _altitude(_hours_later(t, i), lat, lon) - h;
    }
    const h2 = _altitude(_hours_later(t, i+1), lat, lon) - h;
...
function latitude(ms) {
  return _latitude(julian.to(ms)) * DEG;
}
...
function declination(ms) {
  return _declination(julian.to(ms)) * DEG;
}

function right_ascension(ms) {
  return _right_ascension(julian.to(ms)) * DEG;
}

function altitude(ms, lat, lon) {
  return _altitude(julian.to(ms), lat * RAD, lon * RAD) * DEG;
}
